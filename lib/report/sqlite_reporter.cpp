...
void sqlite_reporter::start(query_graph &QG, const std::string &filename,
...
void sqlite_reporter::enter_first_branch() { reporter::enter_first_branch(); }

void sqlite_reporter::leave_first_branch() { reporter::leave_first_branch(); }

void sqlite_reporter::enter_second_branch() { reporter::enter_second_branch(); }

void sqlite_reporter::leave_second_branch() { reporter::leave_second_branch(); }

void sqlite_reporter::bisection_start(query_graph &QG) {
    sqlite3_stmt *stmt = prepare(
        "INSERT INTO `bisection` (`rid`, `branch`, `nodes`, `edges`) VALUES "
        "(?, ?, ?, ?);");
...
void sqlite_reporter::bisection_finish(query_graph &QG,
                                       query_graph &first_subgraph,
                                       query_graph &second_subgraph) {
    double imbalance = std::abs(
        static_cast<long>(first_subgraph.data_graph().number_of_nodes()) -
        static_cast<long>(second_subgraph.data_graph().number_of_nodes()));
    imbalance /= first_subgraph.data_graph().number_of_nodes() +
                 second_subgraph.data_graph().number_of_nodes();
    imbalance *= 100;

    sqlite3_stmt *stmt = prepare(
        "UPDATE `bisection` "
        "SET `p0_nodes` = ?, `p0_edges` = ?, `p1_nodes` = ?, `p1_edges` = ?, "
        "`cut` = ?, `imbalance` = ? "
        "WHERE `id` = ?;");
...
void sqlite_reporter::initial_partitioning_finish(query_graph &QG) {
    sqlite3_stmt *stmt = prepare(
        "UPDATE `bisection` SET `partitioning_time` = ?, `initial_cut` = ? "
        "WHERE `id` = ?;");
...
void sqlite_reporter::refinement_iteration_start(
    query_graph &QG, int nth_iteration, double initial_partition_cost) {
    reporter::refinement_iteration_start(QG, nth_iteration,
                                         initial_partition_cost);
...
void sqlite_reporter::refinement_move_node(
    query_graph &QG, NodeID node, PartitionID from_partition, double gain_total,
    double gain_adjacent, double gain_nonadjacent, bool is_boundary) {
    std::array<NodeID, 2> degrees = {0, 0};
    forall_out_edges(QG.data_graph(), edge, node) NodeID neighbor =
        QG.data_graph().getEdgeTarget(edge);
    ...
    sqlite3_stmt *stmt = prepare(
        ...
        "`iid`, `nodes0`, `nodes1`, `from`, `to`, `gain_total`, "
        "`gain_adjacent`, "
        "`gain_nonadjacent`, `boundary`, `deg_data0`, `deg_data1`, "
        "`deg_query`, "
...
void sqlite_reporter::refinement_iteration_finish(
    query_graph &QG, int num_nodes_exchanged, double resulting_partition_cost) {
    sqlite3_stmt *stmt = prepare(
        "UPDATE `iteration` "
        "SET `resulting_partition_cost` = ?, `num_moved_0to1` = ?, "
        "`num_moved_1to0` = ? "
        "WHERE `id` = ?;");
    sqlite3_bind_double(stmt, 1, resulting_partition_cost);
    ...
    // TODO remove
    std::cout
        << "sqlite_reporter::refinement_iteration_finish(num_nodes_exchanged = "
        << num_nodes_exchanged
        << ", resulting_partition_cost = " << resulting_partition_cost << ")"
        << std::endl;
}
...
void sqlite_reporter::exec_or_throw(const char *sql,
